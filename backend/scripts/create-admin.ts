...
function getLocalOptionalEnv(name: string, fallback = '') {
...
async function createOrUpdateAdmin() {
  ...
  const name = getLocalOptionalEnv('ADMIN_NAME', 'Administrador').trim() || 'Administrador';
  const phone = getLocalOptionalEnv('ADMIN_PHONE', '(00) 00000-0000').trim();
