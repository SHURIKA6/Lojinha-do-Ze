...
function shouldCompress(contentType, contentLength) {
  ...
  const isLargeEnough = contentLength >= MIN_COMPRESSION_SIZE;
 ...
 */
async function compressData(data, _level = COMPRESSION_LEVEL) {
  try {
    ...
    logger.error('Erro na compressão GZIP', error);
    return null;
  }
}
 ...
 */
export function compressionMiddleware(_c, _next) {
  return async (c, next) => {
    ...
    try {
      ...
      logger.error('Erro no middleware de compressão', error);
    }
  };
}
 ...
 */
export function conditionalCompression(minSize = MIN_COMPRESSION_SIZE) {
  return async (c, next) => {
 ...
 */
export function compressRoutes(routes = []) {
  return async (c, next) => {
    const path = c.req.path;
    
    // Verifica se a rota deve ser comprimida
    const shouldCompress = routes.some(route => {
...
/**
 * Middleware de compressão com configurações personalizadas
 */
export function customCompression(options = {}) {
  const {
  ...
  return async (c, next) => {
    ...
    // Verifica tipo de conteúdo personalizado
    const isCompressible = types.some(type => 
      contentType?.includes(type)
    );
    
    // Verifica tamanho mínimo personalizado
    const isLargeEnough = contentLength && parseInt(contentLength) >= minSize;
    
    if (!isCompressible || !isLargeEnough) {
      return;
    }
    
    try {
      const responseBody = await c.res.text();
      const compressedData = await compressData(responseBody, level);
      ...
      logger.error('Erro na compressão personalizada', error);
    }
  };
}