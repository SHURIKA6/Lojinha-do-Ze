 ...
 */
export const ValidationRules: Record<string, ValidationRuleset> = {
 ...
 */
function validateObject(data: any, rules: ValidationRuleset) {
 ...
 */
export function validate(validationRules: ValidationRuleset, options: { sanitize?: boolean } = {}) {
  const {
    sanitize = true
  } = options;
  ...
  return async (c: Context, next: Next) => {
    try {
      ...
      if (sanitize) {
        (c as any).set('validatedData', validation.sanitizedData);
      } else {
        (c as any).set('validatedData', data);
      }
      
      if (validation.warnings.length > 0) {
        (c as any).set('validationWarnings', validation.warnings);
      }
 ...
 */
export function validateCustom(rules: ValidationRuleset, options: { sanitize?: boolean } = {}) {
 ...
 */
export function validateCpfField(fieldName: string = 'cpf') {
  return async (c: Context, next: Next) => {
 ...
 */
export function validatePhoneField(fieldName: string = 'phone') {
  return async (c: Context, next: Next) => {
    const data = (c as any).get('validatedData') || {};
