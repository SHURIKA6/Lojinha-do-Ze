...
class SupplierService {
  ...
  async getSupplier(supplierId) {
    try {
      ...
      let supplier = cacheService.get(`supplier:${supplierId}`);
   ...
   */
  async recordDelivery(supplierId, deliveryData) {
    try {
      const supplier = this.suppliers.get(supplierId);
      if (!supplier) {
        return { success: false, error: 'Fornecedor não encontrado' };
      ...
      logger.error('Erro ao registrar entrega', error);
      return { success: false, error: error.message };
    }
  }
   ...
   */
  async updateSupplierPerformance(supplierId, delivery) {
    const supplier = this.suppliers.get(supplierId);
    if (!supplier) return;
   ...
   */
  calculateSupplierRating(performance) {
  ...
  async getStats() {
    try {
      ...
      logger.error('Erro ao obter estatísticas de fornecedores', error);
      return { success: false, error: error.message };
    }
  }
}
...
export { SUPPLIER_STATUS, SUPPLIER_CATEGORIES };