export async function createSessionRecord(client, session) {
...
export async function findSessionByTokenHash(client, tokenHash) {
  const { rows } = await client.query(
    `SELECT
       s.id AS session_id,
       s.user_id,
...
export async function deleteSessionByTokenHash(client, tokenHash) {
  await client.query('DELETE FROM auth_sessions WHERE token_hash = $1', [tokenHash]);
}

export async function deleteSessionById(client, sessionId) {
  await client.query('DELETE FROM auth_sessions WHERE id = $1', [sessionId]);
}

export async function deleteExpiredSessions(client) {
