...
export async function bootstrapDatabase(databaseUrl: string) {
  ...
  try {
    ...
    const migrations = [
      ...
      // ... outras migrações se existissem em TS ...
    ...
    const users = await sql(`SELECT id FROM users WHERE email = $1`, ['jose@lojinha.com']);
    
    if (users.length === 0) {
      console.log('Criando usuário admin inicial...');
      const adminPassHash = '$2b$10$TQH7yS2G9Wv7G8K/Qo5xGue1X6m55H06E08e6/4H76L9.O5j2hGvG'; // admin123 (hash fixo para bootstrap)
