...
export default function AdvancedDashboard() {
  ...
  const handleExport = async (format) => {
    if (format === 'CSV') {
      try {
        addToast(`Exportação em ${format} iniciada...`, 'info');
        ...
        addToast('Falha ao exportar relatório. Tente novamente.', 'error');
      }
    } else {
      addToast(`A exportação em ${format} ainda não foi implementada para este plano.`, 'warning');
    }
  };
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <Tooltip 
                    ...
                    formatter={(value) => [`${value.toFixed(1)}%`, 'Margem']}
                  />
                  <Area 
                    type="monotone" 
                    dataKey="margem" 
