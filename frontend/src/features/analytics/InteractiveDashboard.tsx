...
export default function InteractiveDashboard() {
  ...
  async function loadAdvancedData() {
    try {
      ...
      toast.error('Não foi possível carregar as previsões baseadas em IA.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
       ...
       <div className="dashboard-card mt-8">
        ...
        <div className="dashboard-card__body--padded">
           <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1rem' }}>
              ...
              {sentiments.length === 0 && (
                <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '2rem', color: 'var(--gray-400)' }}>Nenhuma avaliação para análise.</div>
              )}
           </div>
        </div>
      </div>
