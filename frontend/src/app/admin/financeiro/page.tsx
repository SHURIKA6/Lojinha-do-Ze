...
export default function FinanceiroPage() {
  ...
  const [form, setForm] = useState<{
    ...
    value: number;
    date: string;
  }>({ type: 'receita', category: '', description: '', value: 0, date: '' });
  ...
  const totalEntradas = transactions.filter(t => t.type === 'receita').reduce((s, t) => s + Number(t.value || 0), 0);
  const totalSaidas = transactions.filter(t => t.type === 'despesa').reduce((s, t) => s + Number(t.value || 0), 0);
  const saldo = totalEntradas - totalSaidas;

  // Chart data
  const chartMap: Record<string, { dia: string; receita: number; despesa: number }> = {};
  transactions.forEach(t => {
    ...
    if (t.type === 'receita') chartMap[d].receita += Number(t.value || 0);
    else chartMap[d].despesa += Number(t.value || 0);
  });
  ...
  const handleDelete = async (id: number | string) => {
    const confirmed = await confirm({
      ...
      description: 'Esta ação remove the movimentação financeira.',
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="grid grid-3" style={{ marginBottom: 'var(--space-8)' }}>
        ...
        <div className="metric-card" style={{ '--metric-color': saldo >= 0 ? 'var(--success-500)' : 'var(--danger-500)' } as React.CSSProperties}>
            ...
            <tbody>
              {filtered.map(t => (
                <tr key={t.id}>
                  ...
                  <td style={{ fontWeight: 800, color: t.type === 'receita' ? 'var(--success-600)' : 'var(--danger-500)' }}>{formatCurrency(Number(t.value || 0))}</td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Lançar Transação / DRE"
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            <select className="form-select" value={form.type} onChange={e => setForm({...form, type: e.target.value as Transaction['type']})}>
