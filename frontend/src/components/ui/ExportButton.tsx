...
type ExportFormat = 'pdf' | 'excel' | 'csv' | string;
...
export function ExportButton({
  ...
  const defaultExport = async (format: ExportFormat, exportData: any[], name: string) => {
    switch (format) {
      case 'pdf':
 ...
 */
export function ReportExportButton({ reportType, data, dateRange }: ReportExportButtonProps) {
  const getFilename = () => {
    const date = new Date().toISOString().split('T')[0];
    return `relatorio-${reportType}-${date}`;
  };

  const handleExport = async (format: ExportFormat, exportData: any[]) => {
